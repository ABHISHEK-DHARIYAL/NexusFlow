...
public class MetricsController implements HttpHandler {
    ...
    public MetricsController(WorkerService workerService, InternalAuthFilter authFilter, ObjectMapper objectMapper) {
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        if (!authFilter.authenticate(exchange)) {
            ErrorResponse err = new ErrorResponse("WORKER_UNAUTHORIZED", "Unauthorized metrics access.", correlationId);
        ...
        MetricsResponse metrics = workerService.getMetrics();
        byte[] json = objectMapper.writeValueAsBytes(metrics);
